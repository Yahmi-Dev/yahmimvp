...
// ============================================
// ADVANCED MULTI-PROVIDER AI SYSTEM
// ============================================
...
// Helper: Calculate ESG scores from responses
function calculateESGScores(responses: Record<string, any>) {
  ...
  entries.forEach(([question, answer]) => {
    ...
    if (q.includes('energy') || q.includes('renewable') || q.includes('carbon') || q.includes('emissions')) {
      if (a.includes('76-100') || a.includes('yes') || a.includes('comprehensive')) envScore += 25;
      else if (a.includes('51-75') || a.includes('most')) envScore += 18;
      else if (a.includes('26-50') || a.includes('some')) envScore += 12;
      else envScore += 5;
    }
    
    // Social scoring
    if (q.includes('employee') || q.includes('diversity') || q.includes('community') || q.includes('wellbeing')) {
      if (a.includes('comprehensive') || a.includes('regular') || a.includes('yes')) socScore += 25;
      else if (a.includes('annual') || a.includes('basic')) socScore += 18;
      else if (a.includes('informal') || a.includes('some')) socScore += 12;
      else socScore += 5;
    }
    
    // Governance scoring
    if (q.includes('reporting') || q.includes('governance') || q.includes('policy') || q.includes('compliance')) {
      if (a.includes('third-party') || a.includes('board-level') || a.includes('verified')) govScore += 25;
      else if (a.includes('annual') || a.includes('executive')) govScore += 18;
      else if (a.includes('internal') || a.includes('department')) govScore += 12;
      else govScore += 5;
    }
  });
  ...
  return {
    ...
    social: Math.min(100, socScore),
    governance: Math.min(100, govScore),
    overall: Math.min(100, Math.round((envScore + socScore + govScore) / 3)),
  };
}
